    ...
    public void teleopPeriodic() {
        ...
        if (twoHanded) {
            if (rightStick.getTrigger()) {
                setMotors(leftStick.getY(), rightStick.getY());
            } else if (leftStick.getTrigger()) {
                setMotors(leftStick.getY() * 0.5, rightStick.getY() * 0.5);
            } else {
                setMotors(leftStick.getY() * 0.3, rightStick.getY() * 0.3);
            }

        } else {
            double forward = rightStick.getY();
            double adjust = rightStick.getX();
